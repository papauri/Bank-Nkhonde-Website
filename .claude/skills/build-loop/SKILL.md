...
- **Decide, don't ask — EXCEPT for critical changes.** Ambiguity in ordinary behaviour/UX/wording/defaults → pick what a real savings group would expect, note it, proceed. This includes sensible money-rule *defaults* (e.g. one active loan at a time; arrears/penalties block a new loan by default) — implement the reasonable real-world default, record it, expose it as a retunable setting; do not stall.
- **Ask precisely for CRITICAL design or functionality changes.** A change that materially alters how the app *behaves*, *looks*, or *moves money* — a major page/flow redesign or restructure, a change to a core money rule (not just its default), removing/replacing an existing capability, anything hard to reverse or that the owner would clearly want to sign off — is NOT decided silently. Pause and put **ONE precise question** to the owner: state the exact change, the concrete options, and your recommendation. Then proceed on their answer. When genuinely unsure whether something crosses into "critical", lean toward a tight question over an assumption. (This is a confirm-before-proceed, distinct from a hard BLOCKER — small fixes and additive features never need it.)
